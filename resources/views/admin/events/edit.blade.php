        ...
        <form method="POST" action="{{ route("admin.events.update", [$event->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-collection">
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('cep') ? 'is-invalid' : '' }}" type="text" name="cep" id="cep" value="{{ old('cep', $event->cep) }}" placeholder="00000-000">
                    @if($errors->has('cep'))
                        <div class="invalid-feedback">
                            {{ $errors->first('cep') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('state') ? 'is-invalid' : '' }}" name="state" id="state">
                        ...
                        @foreach(\App\Support\Address\BrazilianStates::all() as $uf => $stateName)
                            <option value="{{ $uf }}" {{ old('state', $event->state) === $uf ? 'selected' : '' }}>{{ $stateName }} ({{ $uf }})</option>
                        @endforeach
                    </select>
                    @if($errors->has('state'))
                        <div class="invalid-feedback">
